    ...
    ILogger<CityPayService> logger)
{
    ...
    private record CityPayMerchantData(string? SessionId, string? Ref, string? Lang = null);
    ...
    private async Task FinalizeApprovedOrderAsync(Order order, Store store)
    {
        ...
        if (order.DiscountCode is not null)
        {
            if (!await discountCodeService.ConfirmUsageAsync(store.Id, order.DiscountCode))
                logger.LogWarning("Discount code {Code} could not be confirmed for CityPay order {OrderId} — it likely hit its usage limit concurrently.", order.DiscountCode, order.Id);
        }
